...
FROM ghcr.io/grafana/grafana-build-tools:1.23.1@sha256:66fb15e42d9b1f3ba68b28d8117c3dd6f598d5975ca8d3a01238054930356df9 AS girls

ARG TARGETOS
ARG TARGETARCH

# Add Grafana Image Renderer Linux Sandbox (GIRLS).
ADD https://github.com/grafana/grafana-image-renderer/releases/download/v5.0.11/grafana-image-renderer-${TARGETOS}-${TARGETARCH} /usr/local/bin/girls
...
RUN <<EOF
  ...
  apk --no-cache add --repository community tini
EOF
...
COPY --from=girls --chown=k6:k6 --chmod=0500 /usr/local/bin/girls /usr/local/bin/girls
