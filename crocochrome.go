...
type Options struct {
	...
	// Defaults to 5222.
	ChromiumPort string
	// DisableNSJail runs chromium directly, instead of using the nsjail sandbox.
...
func (s *Supervisor) launch(ctx context.Context, sessionID string) error {
	...
	if s.opts.UserGroup != 0 {
		nsjailArgs = append(nsjailArgs,
			"--user", strconv.Itoa(s.opts.UserGroup),
			"--group", strconv.Itoa(s.opts.UserGroup),
		)
	}
	...
	var cmd *exec.Cmd
	if s.opts.DisableNSJail {
		logger.Warn("nsjail is disabled, chromium will run unsandboxed in the pod namespace")
		...
		cmd = exec.CommandContext(ctx,
			"nsjail",
			append(append(nsjailArgs, "--", s.opts.ChromiumPath), chromiumArgs...)...,
		)
	}
