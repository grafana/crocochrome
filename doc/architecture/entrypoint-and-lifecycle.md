...
sequenceDiagram
    ...
    main->>sup: Wait() (grace = SessionTimeout + 10s)
...
- **Goroutine B** waits for the context to be cancelled (a signal), then:
  ...
  2. Builds a `graceCtx` with a grace timeout **derived from the session
     timeout**: `graceTime := supervisor.SessionTimeout() + 10*time.Second`,
     ...
     sufficient by construction; the 10s margin covers post-kill teardown.
...
Kubernetes, `terminationGracePeriodSeconds` must be at least that (~320s with
