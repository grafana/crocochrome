...
2. **Integration tests** that build the real container image and drive it with
   real `k6` browser tests via [testcontainers](https://golang.testcontainers.org/), validating the whole stack
   end-to-end.
...
- `NewHeartbeat(t)` writes a small `heartbeat.sh` into `t.TempDir()`. The script
  ...
  FD-leak-into-`exec` problem ([golang/go#22315](https://github.com/golang/go/issues/22315)) that would cause `ETXBSY`.
